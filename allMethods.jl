...
function stJWAS(phenoDataInRef::DataFrame,phenoDataInVal::DataFrame,genoData_All::DataFrame,trait::Int,BayesX::String,piValue::Float64,nChain::Int,nBurnin::Int,nThin::Int,varR::Float64,varG::Float64)
    ...
    if BayesX=="RR-BLUP"
        ...
        else out = runMCMC(model1,phenoRef,Pi=piValue,estimatePi=false,chain_length=nChain,burnin=nBurnin,methods=BayesX,output_samples_frequency=nThin,output_samples_file="MCMC_samples_$BayesX$(piValue).txt");
    end
...
#    sol, convHist = cg(lhs,rhs,tol=1e-12,log=true)
    ...
    testPhenoRows2 = [findall(i -> i == j, phenoData_G5[:ID])[] for j in noPnoGInd];
...
function mtJWAS(phenoDataInRef::DataFrame,phenoDataInVal::DataFrame,genoData_All::DataFrame,nTrait::Int,BayesX::String,piValue,nChain::Int,nBurnin::Int,nThin::Int,varR,varG)
    ...
    refRows = [findall(i -> i == j, phenoData_G4[:ID])[] for j in gpInd]
    phenoRef = phenoData_G4[refRows,:];
    println("covG in ref $(size(phenoRef)) $(cov(convert(Array,phenoRef[[:u1,:u2]])))")
    println("covR in ref $(size(phenoRef)) $(cov(convert(Array,phenoRef[[:e1,:e2]])))")
